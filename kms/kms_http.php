...
class Http
{
	...
	public function send_request($req_inter) {
		if(!$this->keep_alive)
		{
			$this->curl = curl_init();
		}
		...
		if($this->curl == NULL)
		{
			throw new KMSClientException("Curl init failed");
			return;
		}
		...
		curl_setopt($this->curl, CURLOPT_TIMEOUT, $this->connection_timeout );
		...
		if(curl_errno($this->curl)) {
			throw new KMSClientNetworkException(curl_error($this->curl));
		}
